...
struct MacroDefinitionOutputMappingSource {
    uint8_t parameterIndex;
    int32_t multiplier;
    int32_t divider;
    enum MacroCurveType curve;
};

struct MacroDefinitionOutputMapping {
    int8_t ctrl;
    enum MachineParameterType ctrltype;
    int16_t startValue;
    struct MacroDefinitionOutputMappingSource sources[MaxMacroOutputMappingSources];
};
...
struct MacroDefinition {
    ...
    struct MacroDefinitionOutputMapping outputMappings[MaxMacroOutputMappings];
};
...
struct GetMacroDefinitionsPageResponse {
    ...
    struct MacroDefinition macroDefinitions[MaxMacrosPerPage];
};
