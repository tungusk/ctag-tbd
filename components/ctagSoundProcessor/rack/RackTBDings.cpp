...
void RackTBDings::Init(const GrooveBoxRackInitData *initdata) {
    // Init is called once at GrooveBoxRack construction time, before the audio
...
void RackTBDings::Process(const GrooveBoxRackProcessData &data) {
    (void)data;
    if (!enabled) return;
    ...
    // TBD-16's CC handler (GrooveBoxRack.cpp:845) maps wire 0..127 → 0..4096
